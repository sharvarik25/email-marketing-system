...
def establishConnection():
    credentials = pyodbc.connect('DRIVER='+cfg.azure[0]['driver']+';SERVER='+cfg.azure[0]['server']+';DATABASE='+cfg.azure[0]['database']+';UID='+cfg.azure[0]['username']+';PWD='+cfg.azure[0]['password'])
    cursor = credentials.cusror()
    return cursor

def getData():
    cursor = establishConnection()
    query = cursor.execute("SELECT * FROM [dbo].[customers]")
    query_job = query.fetchall()  # API request - starts the query
    customer = query_job.to_dataframe()
#     customer.head()
    query = ("SELECT * FROM [dbo].[movies]")
    query_job = query.fetchall()  # API request - starts the query
...
def recommendMovie(userId):
    ...
    api_key = cfg.model[0]['key']  # Replace this with the API key for the web service
